...
1. [강의 개요](#1-강의-개요) — 학습=최적화 문제, 1·2강 연결, 오늘의 목표
2. [미분 복습](#2-미분-복습) — 접선 기울기, GD 방향계, 기본 공식표(역전파 공구상자), 수치 미분·접선 시각화
3. [편미분과 그래디언트](#3-편미분과-그래디언트) — 3D 슬라이스, 벡터장, 1강 GD vs 신경망 GD, 합성함수 관점
4. [Chain Rule](#4-chain-rule) — 단변수(곱), 다변수(경로별 합), 계산 그래프 적용
5. [Sigmoid 미분](#5-sigmoid-미분) — 3단계 유도, forward 재사용, Vanishing Gradient 예고
6. [Gradient Descent](#6-gradient-descent) — 1D/2D, Learning Rate, non-convex, gradient_descent.py 코드 연결
7. [신경망/Backprop과의 연결](#7-신경망backprop과의-연결) — 계산 그래프, 역전파 이유, 전체 절차, 효율성, 결론
8. [정리 및 연습 아이디어](#8-정리-및-연습-아이디어) — 핵심 3개, 헷갈리기 쉬운 2개, 다음 강 연결 3개, 연습 과제
...
> **이 절의 결론:**
> Chain Rule + 계산 그래프 + Gradient Descent = **신경망 학습(Backpropagation)** 의 전부다.

---

### 7.1 계산 그래프 = 신경망의 그림

신경망의 forward pass는 계산 그래프로 표현된다.

| 계산 그래프 개념 | 신경망 대응 |
|----------------|------------|
| **노드(node)** | 중간 값 \(z^{(l)}, a^{(l)}\) 또는 \(L\) |
| **엣지(edge)** | "이 값이 저 값으로 들어가는" 함수 관계 → 편미분의 경로 |
| **왼→오 탐색** | Forward pass: 값 계산 |
| **오→왼 탐색** | Backward pass: gradient 계산 |

**슬라이드 핵심 문장:**
> "신경망의 각 뉴런은 계산 그래프의 노드, 각 연결은 편미분의 경로."

따라서: **계산 그래프 + Chain Rule = Backpropagation**

---

### 7.2 2-2-1 신경망 — forward pass 전개
...
| 노드 | 값 | 역할 |
|------|-----|------|
| \(z^{(1)}\) | \(W^{(1)}x + b^{(1)}\) | 1층 선형 결합 |
| \(a^{(1)}\) | \(\sigma(z^{(1)})\) | 1층 활성화 출력 |
| \(z^{(2)}\) | \(W^{(2)}a^{(1)} + b^{(2)}\) | 2층 선형 결합 |
| \(a^{(2)} = \hat{y}\) | \(\sigma(z^{(2)})\) | 최종 출력 |
| \(L\) | \(\ell(\hat{y}, y)\) | 손실 스칼라 |

Forward pass에서 이 모든 **중간값 \((z, a)\)를 저장**해 둔다 — backward에서 재사용할 것이기 때문이다.

---

### 7.3 한 weight의 영향 — Chain Rule 5항 전개
...
| 항 | 값 | 의미 |
|----|-----|------|
| ① \(\partial L/\partial a^{(2)}\) | loss 함수에서 계산 | **출발점** — Loss가 출력에 얼마나 민감한가 |
| ② \(\partial a^{(2)}/\partial z^{(2)}\) | \(\sigma(a^{(2)})(1-\sigma(a^{(2)}))\) | 2층 sigmoid 미분 (forward값 재사용) |
| ③ \(\partial z^{(2)}/\partial a^{(1)}\) | \(= W^{(2)}\) | 2층 가중치 — "1층 출력이 2층 입력에 미치는 영향" |
| ④ \(\partial a^{(1)}/\partial z^{(1)}\) | \(\sigma(a^{(1)})(1-\sigma(a^{(1)}))\) | 1층 sigmoid 미분 |
| ⑤ \(\partial z^{(1)}/\partial W^{(1)}\) | \(= x\) | **직접 영향** — 가중치가 자기 뉴런 입력에 미치는 영향 |

> **맨 오른쪽 항(⑤):** 해당 weight의 "직접 영향" (입력 \(x\) 자체)
> **왼쪽으로 갈수록:** 그 뉴런의 출력이 상위 레이어를 거쳐 \(L\)에 전달되는 "간접 영향"

각 항은 단변수 Chain Rule 한 번이고, 5개의 곱이 \(W^{(1)}\)의 전체 영향을 준다.

---

### 7.4 왜 "역"전파인가 — 순방향이 아닌 역방향이어야 하는 두 이유

#### 이유 ①: 오른쪽 결과를 재사용해야 중복 계산이 없다

\(\partial L/\partial a^{(2)}\)를 먼저 구해 두면,
그것을 이용해 \(\partial L/\partial z^{(2)}\), 다시 \(\partial L/\partial a^{(1)}\) … 순서로 왼쪽으로 넘길 수 있다.

\[
\frac{\partial L}{\partial a^{(1)}} = \frac{\partial L}{\partial z^{(2)}} \cdot W^{(2)}
\quad\leftarrow\quad \frac{\partial L}{\partial z^{(2)}} \text{는 이미 계산됨}
\]

오른쪽 gradient가 먼저 완성되어야 왼쪽 gradient를 만들 수 있다.
→ **역방향으로 전파해야 재사용이 가능하다.**

#### 이유 ②: 우리가 원하는 것은 항상 "\(L\)에서 출발한 민감도"이다

구하고 싶은 것: \(\partial L/\partial W^{(l)}\) — "이 weight가 **손실**에 얼마나 민감한가"

\(L\)에서 출발해야 이 질문에 직접 답할 수 있다.
순방향으로 가면, 각 weight가 나중에 \(L\)에 어떻게 영향을 줄지 알기 위해 미래를 매번 재계산해야 한다 → **비효율**.

**결론:**

| | 순방향(Forward) | 역방향(Backward) |
|--|---------------|----------------|
| 계산 대상 | 값 \(z, a\) | Gradient \(\partial L/\partial \cdot\) |
| 시작점 | 입력 \(x\) | 손실 \(L\) (\(\partial L/\partial L = 1\)) |
| 재사용 | 중간값 저장 | 오른쪽 gradient 재사용 |
| 방향 | 왼 → 오른쪽 | **오른쪽 → 왼쪽** |

---

### 7.5 Backpropagation 전체 절차 — 계산 그래프 버전
...
**역방향 탐색 (각 노드 \(v\)에 대해 오른쪽→왼쪽):**

```
각 노드 v에 대해:
  1. v에서 출발하는 엣지 수집 (v → u₁, v → u₂, ...)
  2. 다변수 Chain Rule (경로 합):
     ∂L/∂v = Σₖ (∂L/∂uₖ) · (∂uₖ/∂v)
              ↑ 이미 계산된 값  ↑ 로컬 편미분
  3. ∂L/∂v 저장
```
...
```mermaid
flowchart LR
    A["Forward Pass<br/>z, a 계산·저장"] --> B["Loss L 계산"]
    B --> C["Backward Pass<br/>∂L/∂v 역방향 전파"]
    C --> D["GD 업데이트<br/>W ← W - η·∂L/∂W"]
    D -->|"다음 iteration"| A
```

---

### 7.6 효율성 — 왜 O(파라미터 수)인가

| 방법 | 복잡도 | 이유 |
|------|--------|------|
| 파라미터별 forward 재실행 | \(O(\text{파라미터 수} \times \text{데이터 크기})\) | 매번 전체 네트워크 재실행 |
| **Backpropagation** | **\(O(\text{파라미터 수})\)** | forward의 **중간 결과를 재사용** |

**Backpropagation이 효율적인 이유 한 문장:**
> **계산 그래프** 위에서 한 번의 forward pass로 저장한 **중간 결과** \((z, a)\)를 backward pass가 재사용하기 때문에, 모든 파라미터의 gradient를 **O(파라미터 수)** 연산으로 동시에 계산할 수 있다.

만약 각 파라미터마다 forward를 새로 실행했다면:

\[
O(\text{파라미터 수} \times \text{데이터 크기}) \quad \text{— 수백만 배 느림}
\]

### 7.7 오늘 도구들의 역할 요약 — Chain Rule + GD = 신경망 학습

| 수학 도구 | Backpropagation에서의 역할 |
|-----------|--------------------------|
| 미분 \(f'(x)\) | GD의 방향/속도계 — 접선 기울기로 이동 방향 결정 |
| 편미분 \(\partial f/\partial x_i\) | 각 파라미터 \(W_{ij}\)에 대한 \(\partial L/\partial W_{ij}\) 계산 |
| 그래디언트 \(\nabla L\) | 모든 편미분을 하나의 업데이트 벡터로 묶음 |
| Chain Rule (단변수 곱) | 한 경로 위에서 층별 편미분을 연쇄 곱 |
| Chain Rule (다변수 합) | 여러 경로가 한 노드에서 합쳐질 때 기여 합산 |
| Sigmoid 미분 \(\sigma(1-\sigma)\) | 각 층 활성화 통과 시 gradient 변화량 — forward값 재사용 |
| Gradient Descent | 계산된 \(\nabla L\)로 \(\theta \leftarrow \theta - \eta\nabla L\) 업데이트 |

**최종 결론 한 줄:**

> **Chain Rule(합성함수 편미분의 연쇄 곱·합) + 계산 그래프(순·역방향 탐색) + Gradient Descent(파라미터 업데이트) = Backpropagation = 신경망 학습.**

---

### 7.8 (선택적 심화) 최적화를 더 넓은 시각으로 보면

이 절은 강의 본문 밖의 심화 관점이다. 시험 범위가 아니지만, "왜 GD만이 유일한 답인가?"를 생각해 본 적 있다면 읽어볼 만하다.

**Bayesian 관점:** GD가 찾는 것은 손실 \(L(\theta)\)를 최소화하는 점이다. Bayesian 관점에서는 이것을 파라미터의 사후 분포 \(p(\theta | \text{data})\)의 MAP(Maximum A Posteriori) 추정으로 해석할 수 있다. 손실함수가 cross-entropy + L2 정규화라면, 이는 가우시안 사전분포 하의 log-posterior를 최대화하는 것과 수학적으로 동치다. GD는 그 고차원 확률 지형 위를 점 추정으로 탐색하는 알고리즘인 셈이다.

**더 넓은 최적화 관점:** GD 외에도 Newton's method(2차 도함수 활용), Conjugate Gradient, Evolutionary Algorithms 등 다양한 최적화 방법이 있다. 신경망에서 GD 계열이 표준이 된 이유는 단순히 "빠르다"가 아니라, Backprop과 결합했을 때 고차원에서 \(O(\text{파라미터 수})\)로 gradient를 계산할 수 있기 때문이다. 2차 도함수(Hessian)를 쓰는 방법은 파라미터 수의 제곱 메모리가 필요해 현실적으로 불가능하다.
...
1. **미분 → 편미분 → 그래디언트: 차원 확장의 흐름**
   1D 함수의 기울기(미분) → 다변수 함수에서 한 방향씩(편미분) → 모든 방향을 한 벡터로(그래디언트).
   이 흐름이 "파라미터 2개에서 수백만 개로" 같은 구조를 유지하며 확장된다.

2. **Chain Rule(단변수 곱 + 다변수 합) + 계산 그래프**
   한 경로 내부에서는 편미분을 곱하고, 경로가 여러 개이면 기여를 합한다.
   신경망을 계산 그래프로 보면, 이 규칙을 각 노드에 적용하는 것이 역전파의 전체 절차다.

3. **Backpropagation = Chain Rule + Gradient Descent를 신경망에 적용한 것**
   알고리즘 자체(\(\theta \leftarrow \theta - \eta\nabla L\))는 1강 선형회귀 GD와 동일하다.
   달라진 것은 \(\nabla L\)을 구하는 방법이 "직접 편미분 한 줄"에서 "합성 구조를 역방향으로 거스르는 체인룰"로 바뀐 것뿐이다.

---

### 8.2 헷갈리기 쉬운 포인트 2개

1. **"3D 그래프의 x, y는 입력 피처가 아니라 파라미터(가중치)다"**
   슬라이드의 3D 손실 그릇에서 두 가로 축은 \((w_1, w_2)\) — 파라미터다. 입력 데이터 \(x\)는 학습 중에 고정이다.
   파라미터가 수백만 개가 되면 시각화가 불가능하지만, 수식 \(\theta \leftarrow \theta - \eta\nabla L\)은 그대로 통한다.

2. **"선형회귀 GD와 신경망 GD는 다른 알고리즘이 아니다"**
   업데이트 식 \(\theta \leftarrow \theta - \eta\nabla_\theta L\)은 완전히 동일하다.
   달라진 것은 ① 파라미터 차원(2개 → 수백만 개)과 ② gradient 계산 방식(직접 편미분 → Backprop 체인룰)뿐이다.
   퍼셉트론 1개짜리 신경망은 로지스틱 회귀와 완전히 동일하고, 층이 깊어질수록 합성함수 구조가 복잡해진다.

---

### 8.3 다음 강에서 이어질 노드 3개

| 주제 | 내용 | 연결 |
|------|------|------|
| **Gradient Check (Week 11)** | 수치 미분 ≈ 해석적 gradient로 역전파 구현 검증. `numerical_derivative` 함수가 그대로 쓰인다. | §2.5 수치 미분 |
| **Vanishing Gradient (Week 12)** | Sigmoid 층이 \(L\)개이면 gradient가 최대 \(0.25^L\)으로 소멸. ReLU·Batch Normalization으로 해결. | §5 Sigmoid 미분 ② 주목할 점 |
| **고급 Optimizer (Week 12~)** | Momentum·Adam은 2D GD에서 타원 진동 비효율을 해결하기 위해 발전. GD의 수식을 확장한 것. | §6.6 2D GD 타원 진동 |
...
| 개념 | 역할 | 이전 강 연결 |
|------|------|-------------|
| 미분 \(f'(x)\) | GD의 방향·속도계 | 1강 MSE 기울기 |
| 편미분 \(\partial f/\partial x_i\) | 파라미터별 독립 변화율 | 1강 \(\partial J/\partial m,\;\partial J/\partial b\) |
| 그래디언트 \(\nabla L\) | 편미분 벡터화, GD의 입력 | 1강 GD 일반화 |
| Chain Rule (단변수) | 한 경로 내 층별 편미분 곱 | §3.8.2 합성함수 구조 |
| Chain Rule (다변수) | 여러 경로 기여 합산 | §4.5 경로의 합 |
| Sigmoid 미분 \(\sigma(1-\sigma)\) | 활성화 gradient, forward 재사용 | 2강 로지스틱 회귀 |
| Backpropagation | Chain Rule을 계산 그래프에 역방향 적용 | 8강 Forward→Backward 루프 |
| Gradient Descent | \(\theta \leftarrow \theta - \eta\nabla L\) 파라미터 업데이트 | 1강 GD, 6강 `MLPClassifier` |

---

### 8.5 자습용 연습 과제

1. `numerical_derivative(lambda x: np.sin(x), a=np.pi/4)`를 실행하고 해석적 결과 \(\cos(\pi/4)\approx 0.707\)과 비교하라. 차이가 \(10^{-10}\) 이하인가?
2. \(f(x,y)=\ln(x^2+y^2+1)\)의 그래디언트를 손으로 구한 뒤, `gradient_descent_2d`로 GD 경로를 시각화하고 최솟값이 \((0,0)\)으로 수렴하는지 확인하라.
3. \(\text{tanh}(x) = \dfrac{e^x-e^{-x}}{e^x+e^{-x}}\)를 미분해 \(1-\tanh^2(x)\)임을 유도하라. Sigmoid와 달리 최댓값은 얼마이고, Vanishing Gradient 측면에서 어떤 차이가 있는가?
4. 2층 신경망 Chain Rule 5항 \(\partial L/\partial W^{(1)} = ① \cdot ② \cdot ③ \cdot ④ \cdot ⑤\)에서 각 항이 "어느 노드에서 어느 노드로의 편미분"인지 직접 말로 설명해보라.

---

<!-- INVENTORY_MARKERS:20260507_10 -->

## RAG 노드·전사 정렬 마커 인덱스 (20260507_10)

> 이 절은 인벤토리/RAG 파이프라인에서 insert-only로 추가한 마커 인덱스다. 기존 강의록 본문은 삭제·재작성하지 않았다.

> [[NODE:n_ML10.section_01]] 강의 개요
> anchors: [[ANCHOR:ML10.A.4]]
> segments:
> evidence: [[EVID:ev_20260507_10_009]]
>
> [[NODE:n_ML10.section_02]] 미분 복습
> anchors: [[ANCHOR:ML10.A.12]]
> segments: [[SEG:ML10.A.12.local_001]]
> evidence: [[EVID:ev_20260507_10_001]] [[EVID:ev_20260507_10_010]]
>
> [[NODE:n_ML10.section_03]] 편미분과 그래디언트
> anchors: [[ANCHOR:ML10.A.28]]
> segments: [[SEG:ML10.A.28.local_001]]
> evidence: [[EVID:ev_20260507_10_002]] [[EVID:ev_20260507_10_011]]
>
> [[NODE:n_ML10.chain_rule]] Chain Rule
> anchors: [[ANCHOR:ML10.A.52]]
> segments: [[SEG:ML10.A.52.local_001]]
> evidence: [[EVID:ev_20260507_10_003]] [[EVID:ev_20260507_10_012]]
>
> [[NODE:n_ML10.sigmoid]] Sigmoid 미분
> anchors: [[ANCHOR:ML10.A.66]]
> segments: [[SEG:ML10.A.66.local_001]]
> evidence: [[EVID:ev_20260507_10_004]] [[EVID:ev_20260507_10_013]]
>
> [[NODE:n_ML10.gradient_descent]] Gradient Descent
> anchors: [[ANCHOR:ML10.A.76]]
> segments: [[SEG:ML10.A.76.local_001]]
> evidence: [[EVID:ev_20260507_10_005]] [[EVID:ev_20260507_10_014]]
>
> [[NODE:n_ML10.backprop]] 신경망/Backprop과의 연결
> anchors: [[ANCHOR:ML10.A.87]]
> segments: [[SEG:ML10.A.87.local_001]]
> evidence: [[EVID:ev_20260507_10_006]] [[EVID:ev_20260507_10_015]]
>
> [[NODE:n_ML10.section_08]] 정리 및 연습 아이디어
> anchors: [[ANCHOR:ML10.A.98]]
> segments: [[SEG:ML10.A.98.local_001]] [[SEG:ML10.A.98.local_002]]
> evidence: [[EVID:ev_20260507_10_007]] [[EVID:ev_20260507_10_008]] [[EVID:ev_20260507_10_016]]
>
