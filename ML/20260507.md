...
### 3.8.1 흔한 오해 정정 — "3D 그래프의 x, y가 입력 피처인가?"

> **아니다. 슬라이드·ipynb의 그래프에서 두 축 \(x, y\)는 입력 데이터가 아니라 모델 파라미터(가중치)다.**

이게 헷갈리는 이유는 기호가 겹치기 때문이다. 정확히 구분하면:

| 기호 | 역할 | 학습 중 바뀌는가? |
|------|------|-----------------|
| \(x\) (데이터) | 입력 피처 (키·나이 등) | **고정** — 학습이 바꾸는 대상이 아님 |
| \(w_1, w_2\) (파라미터) | 모델 가중치 | **변함** — GD가 매 스텝 업데이트 |
| \(L(w_1, w_2)\) | 손실 (높이) | 파라미터에 따라 달라지는 스칼라 |

슬라이드의 3D 그림에서:
- **가로 두 축** = 파라미터 \(w_1, w_2\) (그릇의 바닥면)
- **세로 축(높이)** = 그 파라미터일 때의 손실 \(L\)

GD는 이 그릇 위에서 경사를 따라 **파라미터 벡터**를 움직이는 것이다.

#### 그렇다면 1강 GD와 지금 GD의 차이는 정확히 무엇인가?

**같은 것:** 구조 자체는 동일하다.  
1강 선형회귀도 이미 파라미터 \((m, b)\) 2개짜리 **다변수 GD**였다.

달라진 것은 단 두 가지다.

| 비교 | 1강 선형회귀 GD | 신경망 GD |
|------|---------------|----------|
| **파라미터 수** | 2개 \((m, b)\) | 수천~수백만 개 \((W^{(1)}, b^{(1)}, \ldots)\) |
| **손실의 구조** | 선형 모델 → 편미분이 닫힌 형태로 단순 | **여러 함수가 합성** → 층마다 Chain Rule로 분해해야 함 |
| **편미분 계산법** | 직접 손으로 유도 가능 | **Backpropagation** 없이는 불가 |
| **손실 곡면 모양** | Convex (그릇 하나, 전역 최솟값 유일) | Non-convex (봉우리·골짜기 여럿, local min 존재) |
...
\(L\)이 \(m\)에 대해 어떻게 변하는지 한 번에 계산 가능:

\[
\frac{\partial L}{\partial m} = \frac{-2}{n}\sum x_i(y_i - mx_i - b)
\]

선형 구조라 합성이 없고, 편미분 결과가 한 줄로 나온다.

**신경망 2층 예시:**

\[
L = \text{loss}\bigl(\sigma(W^{(2)}\cdot\sigma(W^{(1)}x + b^{(1)}) + b^{(2)})\bigr)
\]

\(W^{(1)}\)에 대한 편미분을 구하려면?  
Loss → \(\sigma\) → \(W^{(2)}\) → \(\sigma\) → \(W^{(1)}\)까지 **합성함수를 층마다 거슬러 올라가야** 한다.  
이게 Chain Rule이 필요한 이유이고, 그 절차를 효율화한 것이 역전파다.

#### 왜 지금 슬라이드에서 3D가 나오는가

"파라미터 2개짜리 손실 곡면"을 시각화해서 GD의 동작 방식을 눈으로 보여주기 위함이다.  
파라미터가 수백만 개면 시각화가 불가능하다.  
**3D 그림은 "원리 이해용 단순 모델"이고, 수식 \(\theta \leftarrow \theta - \eta\nabla_\theta L\)은 몇 백만 차원에서도 그대로 통한다.**

```
파라미터 2개  → L(w₁, w₂)  → 3D 그릇 그림으로 GD 시각화 가능
파라미터 100만 개 → L(θ)    → 시각화 불가, 수식만 동일하게 적용
```
