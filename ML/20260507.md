...
> **Gradient Descent에 필요한 수학 도구(\(\nabla L\), Chain Rule)를 확보한다.**

8강(Perceptron·MLP)에서 학습 루프를 `Forward → Loss → Backward → Update`로 소개했다.  
오늘은 그 중 **Backward(역전파)**와 **Update(파라미터 갱신)**가 수학적으로 왜 작동하는지를 해부한다.

```mermaid
flowchart LR
    A["미분<br/>변화율"] --> B["편미분<br/>방향별 변화율"]
    B --> C["그래디언트 ∇f<br/>벡터화"]
    C --> D["Gradient Descent<br/>x ← x - η∇f"]
    A --> E["Chain Rule<br/>합성함수 분해"]
    E --> F["Backpropagation<br/>역방향 연쇄"]
    F --> D
    G["Sigmoid σ(x)"] --> H["σ'=σ(1-σ)<br/>forward 재사용"]
    H --> F
```

### 선행 강의와의 연결

| 이전 강의 | 이번 강의에서 연결되는 지점 |
|----------|--------------------------|
| 1강 선형회귀 GD | \(m, b\)에 대한 MSE 편미분 → 오늘의 다변수 그래디언트로 일반화 |
| 2강 로지스틱 회귀 | sigmoid 함수가 여기서 등장 → 오늘 \(\sigma'(x) = \sigma(1-\sigma)\) 정당화 |
| 8강 MLP | Forward→Loss→Backward→Update 루프 → 오늘 수식으로 채움 |

### 후속 강의 예고

- **Week 11:** Gradient check (수치 미분으로 역전파 검증)
- **Week 12:** Vanishing Gradient 문제 · ReLU · Batch Normalization
...
미분은 **두 가지 정보**를 동시에 담는다.

| 정보 | 의미 | 신경망 학습에서의 역할 |
|------|------|----------------------|
| **부호** (+/−) | 함수가 증가 중인지 감소 중인지 | 파라미터를 어느 **방향**으로 이동할지 결정 |
| **크기** (\|f'(x)\|) | 기울기가 얼마나 가파른가 | learning rate와 곱해져 **이동 거리** 결정 |
...
| 규칙 | 수식 | 예시 |
|------|------|------|
| 거듭제곱 | \((x^n)' = n x^{n-1}\) | \((x^3)' = 3x^2\) |
| 지수 | \((e^x)' = e^x\) | \((e^{2x})' = 2e^{2x}\) (chain rule 포함) |
| 로그 | \((\ln x)' = \tfrac{1}{x}\) | \((\ln x^2)' = \tfrac{2}{x}\) |
| 곱셈 | \((fg)' = f'g + fg'\) | \((xe^x)' = e^x + xe^x\) |
| 나눗셈 | \(\left(\tfrac{f}{g}\right)' = \tfrac{f'g - fg'}{g^2}\) | §5 Sigmoid 미분에서 직접 사용 |

> **연결:** 나눗셈 법칙은 §5에서 \(\sigma(x) = (1+e^{-x})^{-1}\)을 미분할 때 바로 쓰인다.
