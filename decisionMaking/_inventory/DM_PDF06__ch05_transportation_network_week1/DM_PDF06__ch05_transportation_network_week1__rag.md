# DM_PDF06 — Ch.5 수송계획과 네트워크 분석 최종 RAG 튜터 문서
...
| node_id | 핵심 개념 | 역할 |
|---|---|---|
| `n_DM_PDF06.transportation_problem` | 수송문제 | 여러 공급지에서 여러 수요지로 얼마를 보낼지 결정해 총 수송비를 최소화하는 LP 특수형이다. |
| `n_DM_PDF06.transshipment_problem` | 경유수송문제 | 공급지와 수요지 사이에 경유지가 존재하고, 각 노드에서 유입과 유출의 균형을 맞추는 네트워크 LP다. |
| `n_DM_PDF06.assignment_problem` | 할당문제 | 각 작업을 각 자원에 1:1로 배정해 총 비용을 최소화하거나 효용을 최대화하는 0-1 네트워크 특수형이다. |
| `n_DM_PDF06.network_flow` | 네트워크 흐름 | 노드와 arc로 구성된 시스템에서 흐름량을 결정하고 보존/용량/비용 제약을 만족시키는 모형군이다. |
...
- extraction risk pages: 9. 현재 문서의 단정은 추출된 텍스트 근거에 한정한다.
