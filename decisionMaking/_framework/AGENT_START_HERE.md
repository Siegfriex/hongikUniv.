...
**DecisionMaking PDF-only 우선 규칙:** 이번 작업의 입력은 `decisionMaking/pdf_sources/DM_PDF01`~`DM_PDF08` PDF 8개다. 날짜 기반 `lecture_id`를 추정하지 말고, `PDF_ONLY_POLICY.md`, `FINAL_RAG_TUTOR_PERSONA.md`, `pdf_sources/SOURCE_MAP.md`를 우선한다.
...
_framework/
  ...
  PDF_ONLY_POLICY.md       ← DecisionMaking PDF-only 배치 우선 정책
  FINAL_RAG_TUTOR_PERSONA.md ← 최종 RAG 1:1 튜터 페르소나
...
| 4 | `PDF_ONLY_POLICY.md` + `FINAL_RAG_TUTOR_PERSONA.md` | 이번 PDF-only 배치의 우선 규칙 |
| 5 | `PIPELINE.md` | 한 강 → 5단계 체크리스트 |
| 6 | `SCHEMAS.md` + `CONVENTIONS.md` | sidecar 스키마·id 규약 |
| 7 | `prompts/01..05` | 단계별로 세션에 넣을 프롬프트 |
| 8 (필요 시) | `CODEBASE_TOUR.md`, `HOW_TO_RUN.md`, `COURSE_OVERRIDES.md` | 도구·명령·새 과목 추가법 |

---

## 3. “지금 바로 한 강을 시작” 절차 (5분짜리)

### 3.0 DecisionMaking PDF-only 배치

1. `pdf_sources/SOURCE_MAP.md`에서 `DM_PDFxx` 순서를 확인한다.
2. `pdf_sources/DM_PDFxx_*.pdf`를 절대 원본으로 보고 수정하지 않는다.
3. `pdf_transcripts/DM_PDFxx__<slug>__full_transcript.md`를 먼저 만든다.
4. `_inventory/DM_PDFxx__<slug>/` 아래 sidecar를 만든다.
5. 최종 `*_rag.md`는 `FINAL_RAG_TUTOR_PERSONA.md`의 1:1 튜터 페르소나를 따른다.
...
DecisionMaking PDF-only 배치에서는 위 1번을 `DM_PDFxx` PDF source, full transcript, `_inventory/DM_PDFxx__<slug>/` 세 경로로 대체한다.
