...
## 0. DecisionMaking PDF-only override

이번 `decisionMaking` PDF 8개 배치에서는 날짜 개념을 폐기한다. 원본 PDF 파일명에 신뢰 가능한 수업 날짜가 없으므로, `lecture_id = YYYYMMDD_N` 규칙을 억지 적용하지 않는다.

- source id: `DM_PDF01` ~ `DM_PDF08`
- 순서: 사용자가 지정한 8개 PDF 경로 순서
- inventory folder: `decisionMaking/_inventory/DM_PDFxx__<slug>/`
- transcript anchor: `DM_PDFxx:pNNN:LNNN`
- 원본명 매핑 SSOT: `decisionMaking/pdf_sources/SOURCE_MAP.md`

날짜가 나중에 확인되더라도 기존 `DM_PDFxx` id는 유지하고, `manifest.json`에서 dated lecture와 연결만 추가한다.
