...
**DecisionMaking PDF-only override:** `decisionMaking` PDF 8개 배치에서는 위 날짜 기반 `lecture_id`를 쓰지 않는다. `CONVENTIONS.md` §0에 따라 `DM_PDFxx__<slug>` 폴더와 `DM_PDFxx:pNNN:LNNN` 앵커를 사용한다.
