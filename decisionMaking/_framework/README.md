...
**이번 DecisionMaking 배치의 우선 규칙:** 새 원자료는 PDF 8개뿐이다. 날짜 기반 강의 ID는 추정하지 않고, `DM_PDF01`~`DM_PDF08` 순서를 사용한다. 최종 RAG 문서는 `FINAL_RAG_TUTOR_PERSONA.md`의 1:1 OR/MS 튜터 페르소나를 따른다.
...
DecisionMaking PDF-only 배치는 예외다. 필수 입력은 `pdf_sources/DM_PDFxx_*.pdf`이며, full transcript md를 먼저 생성한 뒤 그 transcript가 content SSOT 역할을 한다.
