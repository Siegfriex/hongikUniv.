...
> **DecisionMaking PDF-only 배치:** 최종 `*_rag.md`는 단순 검색 인덱스가 아니라 1:1 OR/MS 튜터가 사용할 학습 문서다. 반드시 `../FINAL_RAG_TUTOR_PERSONA.md`를 함께 적용하고, 출력 경로는 `_inventory/DM_PDFxx__<slug>/DM_PDFxx__<slug>__rag.md`를 사용한다.
...
DecisionMaking PDF-only 배치에서는 여기에 **전담 튜터 문서 작성자** 역할이 추가된다. 각 핵심 노드는 `한 줄 정의 -> 쉬운 직관 -> 수식/모형 -> 강의 예제 -> 자주 하는 실수 -> 선행/후속/유사 연결` 구조를 가져야 한다.
...
- **출력 파일 1개:** 기본은 `_inventory/<lecture_id>/{YYYYMMDD}_{N}강_래그.md` (manifest의 annotated 파일명과 정합). DecisionMaking PDF-only 배치는 `_inventory/DM_PDFxx__<slug>/DM_PDFxx__<slug>__rag.md`.
